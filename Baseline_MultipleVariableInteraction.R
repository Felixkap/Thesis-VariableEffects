...
sim_once <- function(k, N, num.trees, cor, formula, node_size){
  ...
  names(result_list[['pdp_list']]) <- paste0('x', 1:n_vars)
  ...
  for (v in 1:n_vars) {
    ...
    pdp <- FeatureEffect$new(predictor = predictor, 
                             feature = paste0('x.', v),
                             method = "pdp", 
                             grid.points = seq(-3,3, length.out=20))
 
    result_list[['pdp_list']][[paste0('x', v)]] <- pdp$results[,-ncol(pdp$results)]
...
sim_multi <- function(scenario){
  ...
  res <- replicate(n = repeats,
                   expr = sim_once(k=k, N = N, num.trees = num.trees,
                                   cor = cor, formula = formula, node_size = node_size))
  

  
  
  ### Distribution of Test Statistics
  res_scenario <- list(formula=formula, N=N, k=k, cor=cor, num.trees=num.trees, node_size=node_size,
...
# ###### Simulation Setup
# n <- c(400) ; num.trees <- c(2000) ; repeats <- 200; cor <- c(0) 
# k <- c(1); node_size <- c(5, 50)
# formulas <- c("2*x.1+4*x.2-0.5*x.3+2.2*x.4")
# scenarios <- data.frame(expand.grid(n, num.trees, formulas, repeats, cor, k, node_size))
# colnames(scenarios) = c("N", "N_Trees", "Formula", "Repeats", "Correlation", "k", "Node_Size")
